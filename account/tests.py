...
class TestUserListView:
    ...
    def test_user_list_ordering(self):
        ...
        if results:
            assert all(results[i]['date_joined'] <= results[i + 1]['date_joined'] for i in range(len(results) - 1))
    ...
    def test_user_list_search(self):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert any(user['username'] == self.normal_user.username for user in response.data['results'])
    ...
    def test_retrieve_authenticated_owner(self):
        ...
        # Assert response
        assert response.status_code == status.HTTP_200_OK
        assert response.data['id'] == self.admin_user.id  # Example assertion based on serializer output
...
class TestUserAuthEndpoints:
    ...
    def test_user_login_invalid_credentials(self):
        ...
        response = self.client.post('/user/login/', {'username': 'invalid_user', 'password': 'invalid_password'})
