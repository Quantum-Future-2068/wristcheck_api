...
class TestWatchVisitRecordViewSet:
    list_url = reverse('wishlist-list')  # adjust URL name if necessary
    ...
    def setup(self, db):
        ...
        self.admin_user_watch = WishlistFactory(user_id=self.admin_user.id, watch_id='watch_id_1')
        self.normal_user = UserFactory(is_staff=False, is_superuser=False)
        self.normal_user.set_password('password')
        self.normal_user.save()
        self.normal_user_watch = WishlistFactory(user_id=self.normal_user.id, watch_id='watch_id_1')
    ...
    def test_retrieve_endpoint_successful(self):
        self.client.login(username=self.admin_user.username, password='password')
        detail_url = reverse('wishlist-detail', kwargs={'pk': self.normal_user_watch.id})
    ...
    def test_retrieve_endpoint_forbidden(self):
        self.client.login(username=self.normal_user.username, password='password')
        detail_url = reverse('wishlist-detail', kwargs={'pk': self.admin_user_watch.id})
        response = self.client.get(detail_url)
        assert response.status_code == status.HTTP_403_FORBIDDEN
